...
> {
  ...
  getAllWithPagination: QueryHandle<
    ...
    PaginateResult<T>
  > = async ({ query, paginateOptions = {}, projection, select }) => {
    const filterQuery = this.getFilterQuery(query);
  ...
  aggregateWithPagination = async <R>(args: {
    ...
    const out = await Model.aggregatePaginate(aggregation, {
      ...
      ...(() => {
        /**
         * if the pipeline has a $sort stage
         * and the paginateOptions has a sort
         * then we need to remove the sort from the paginateOptions
         * because the sort from the paginateOptions has priority
         */

        //@ts-expect-error ignore
        const hasSortAggregate = pipeline.some((stage) => !!stage.$sort);
        const removeSortFromPagination = hasSortAggregate && paginateOptions.sort;
        return removeSortFromPagination ? { sort: undefined } : {};
      })()
    });
